    ...
    <div className="w-64 bg-slate-900 min-h-screen text-slate-300 p-4 flex flex-col shrink-0 border-r border-slate-800">
      ...
      <nav className="space-y-2 flex-1">
        ...
        <div>
          ...
          {/* SUBMENU */}
          <div className={`ml-4 mt-2 space-y-1 overflow-hidden transition-all duration-300 ${menuEquipAberto ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
            ...
            {/* Cadastro como botão que aciona modal ou rota */}
            <Link to="/equipamentos/novo" className="flex items-center gap-3 p-2.5 rounded-lg text-xs font-black uppercase tracking-wider text-slate-500 hover:text-blue-400 transition-colors">
      ...
      {/* RODAPÉ DO SIDEBAR */}
      <div className="mt-auto p-4 bg-slate-800/40 rounded-2xl border border-slate-800/50 text-center">
        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Operador Logado</p>
        <p className="text-xs font-bold text-white truncate">Eduardo Nascimento</p>
        <div className="mt-3 pt-3 border-t border-slate-700/50 flex justify-center gap-4">
            <button title="Configurações" className="text-slate-500 hover:text-white transition-colors">⚙️</button>
            <button title="Sair" className="text-red-900 hover:text-red-500 transition-colors">🚀</button>
        </div>
      </div>
    </div>
  );
