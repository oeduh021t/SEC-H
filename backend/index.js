...
app.get('/api/itens-estoque', permitirApenas(['admin', 'coordenador', 'tecnico', 'usuario']), (req, res) => {
    const query = `
        SELECT id, nome, descricao, quantidade, valor_unitario, referencia, tipo 
        FROM itens_estoque 
        ORDER BY nome ASC
    `;
    db.query(query, (err, results) => {
...
app.get('/api/solicitacoes-compra', permitirApenas(['admin', 'coordenador', 'tecnico', 'usuario']), (req, res) => {
        ...
        SELECT 
            ...
            (
                SELECT JSON_ARRAYAGG(
                    JSON_OBJECT(
                        'id', sci.id,
                        'insumo_id', sci.insumo_id,
                        'descricao', sci.descricao,
                        'quantidade', sci.quantidade,
                        'valor_estimado', sci.valor_estimado,
                        'subtotal', (sci.quantidade * sci.valor_estimado)
                    )
                )
                FROM solicitacoes_compra_itens sci
                WHERE sci.solicitacao_id = sc.id
            ) AS itens_json
        FROM solicitacoes_compra sc
        LEFT JOIN setores s ON sc.setor_id = s.id
        LEFT JOIN fornecedores f ON sc.fornecedor_id = f.id
    ...
    db.query(query, (err, results) => {
        ...
        const formatados = (results || []).map(r => {
            let itens = [];
            try {
                itens = typeof r.itens_json === 'string' ? JSON.parse(r.itens_json) : (r.itens_json || []);
            } catch (e) { itens = []; }
            return { ...r, itens };
        });
...
// 2. Detalhes de 1 solicitação (para edição e impressão A4)
app.get('/api/solicitacoes-compra/:id', permitirApenas(['admin', 'coordenador', 'tecnico', 'usuario']), (req, res) => {
    ...
    const queryHeader = `
        SELECT 
            ...
            u.nome AS solicitante_nome,
            IFNULL(sc.nota_fiscal_numero, nf.numero_nf) AS nota_fiscal_numero, nf.chave_acesso AS nota_fiscal_chave
        FROM solicitacoes_compra sc
        LEFT JOIN setores s ON sc.setor_id = s.id
        LEFT JOIN fornecedores f ON sc.fornecedor_id = f.id
        LEFT JOIN equipamentos e ON sc.equipamento_id = e.id
...
app.post('/api/solicitacoes-compra', permitirApenas(['admin', 'coordenador', 'tecnico', 'usuario']), (req, res) => {
    ...
    db.beginTransaction((err, conn) => {
        ...
        conn.query(queryHeader, [v_setor, v_fornecedor, v_equip, v_chamado, Number(solicitante_id), urgencia || 'Média', motivo.trim(), v_nf, v_orc], (errIns, resultIns) => {
            ...
            const queryItem = `INSERT INTO solicitacoes_compra_itens (solicitacao_id, insumo_id, descricao, quantidade, valor_estimado) VALUES ?`;
            const valuesItens = itens.map(item => [
                solicitacaoId,
                item.insumo_id ? Number(item.insumo_id) : null,
                item.descricao.trim(),
                Number(item.quantidade || 1),
                Number(String(item.valor_estimado || 0).replace(',', '.'))
            ]);
...
app.patch('/api/solicitacoes-compra/:id/status', permitirApenas(['admin', 'coordenador', 'tecnico']), (req, res) => {
    ...
    db.beginTransaction((err, conn) => {
        ...
        if (valor_real !== undefined && valor_real !== null && valor_real !== '') {
            campos.push('valor_real = ?');
            params.push(Number(String(valor_real).replace(',', '.')));
        }

        if (nota_fiscal_numero !== undefined && nota_fiscal_numero !== null) {
            campos.push('nota_fiscal_numero = ?');
            params.push(String(nota_fiscal_numero).trim());
        }

        if (status === 'Financeiro') {
            campos.push('user_financeiro_id = ?', 'data_financeiro = NOW()');
            params.push(usuario_id);
        } else if (status === 'Diretoria' || status === 'Aprovado') {
            campos.push('user_diretoria_id = ?', 'data_diretoria = NOW()');
            params.push(usuario_id);
        }

        params.push(id);
        const queryUpdate = `UPDATE solicitacoes_compra SET ${campos.join(', ')} WHERE id = ?`;
...
// ==========================================
// 🔍 1. DADOS DO EXTRATO CONSOLIDADO DO FORNECEDOR (JSON PARA A TELA)
// ==========================================
app.get('/api/fornecedores/:id/extrato-completo', (req, res) => {
    const fornecedorId = req.params.id;
    const { data_inicio, data_fim } = req.query;

    const dataInicio = data_inicio || '2000-01-01';
    const dataFim = data_fim || '2099-12-31';

    const queryFornecedor = `SELECT *, COALESCE(nome_fantasia, razao_social) AS nome_referencia FROM fornecedores WHERE id = ?`;

    db.query(queryFornecedor, [fornecedorId], (errForn, rowsForn) => {
        if (errForn || rowsForn.length === 0) {
            return res.status(404).json({ error: 'Fornecedor não encontrado.' });
        }

        const fornecedor = rowsForn[0];
        const nomeParaBusca = fornecedor.nome_referencia;
        ...
        const queryOrcamentos = `
            SELECT o.id, o.codigo_orcamento, o.data_emissao, o.valor_total, o.status, o.tipo_orcamento,
                   o.observacoes, o.anexo_url, s.nome AS setor_nome,
                   (SELECT COUNT(*) FROM orcamentos_externos_itens oi WHERE oi.orcamento_id = o.id) AS total_itens
            FROM orcamentos_externos o
            LEFT JOIN setores s ON o.setor_id = s.id
            WHERE o.fornecedor_id = ?
              AND DATE(o.data_emissao) BETWEEN ? AND ?
            ORDER BY o.id DESC
        `;

        const queryNotas = `
            SELECT id, numero_nf, serie, chave_acesso, data_emissao, data_recebimento, 
                   valor_total, descricao, status, url_danfe, url_xml
            FROM notas_fiscais
            WHERE fornecedor_id = ?
        ...
        const queryDespesas = `
            SELECT dp.id, dp.codigo_referencia, dp.categoria, dp.descricao, dp.valor_total,
                   dp.forma_pagamento, dp.data_competencia, dp.status_pagamento, s.nome AS setor_nome
            FROM despesas_prediais dp
            LEFT JOIN setores s ON dp.setor_id = s.id
            WHERE dp.fornecedor_id = ?
              AND DATE(dp.data_competencia) BETWEEN ? AND ?
            ORDER BY dp.id DESC
        `;

        const querySaidasProntuario = `
            SELECT eh.id, eh.equipamento_id, eh.status_anterior, eh.status_novo, 
                   eh.descricao_log, eh.tecnico_nome, eh.data_movimentacao,
                   ...
                   END AS situacao_custodia
            FROM equipamentos_historico eh
            INNER JOIN equipamentos e ON eh.equipamento_id = e.id
            LEFT JOIN setores s ON e.setor_id = s.id
            WHERE (eh.descricao_log LIKE CONCAT('%', ?, '%') OR eh.descricao_log LIKE CONCAT('%Fornecedor ID: ', ?, '%'))
              AND (eh.descricao_log LIKE '%SAÍDA EXTERNA%' OR eh.descricao_log LIKE '%SAIDA EXTERNA%' OR eh.descricao_log LIKE '%Enviado para%')
              AND DATE(eh.data_movimentacao) BETWEEN ? AND ?
            ORDER BY eh.data_movimentacao DESC
        `;

        db.query(queryChamados, [fornecedorId, fornecedorId, dataInicio, dataFim], (errCh, chamados) => {
            if (errCh) return res.status(500).json({ error: errCh.message });

            db.query(queryOrcamentos, [fornecedorId, dataInicio, dataFim], (errOrc, orcamentos) => {
                if (errOrc) return res.status(500).json({ error: errOrc.message });

                db.query(queryNotas, [fornecedorId, dataInicio, dataFim], (errNf, notas) => {
                    if (errNf) return res.status(500).json({ error: errNf.message });

                    db.query(queryDespesas, [fornecedorId, dataInicio, dataFim], (errDp, despesas) => {
                        if (errDp) return res.status(500).json({ error: errDp.message });

                        db.query(querySaidasProntuario, [nomeParaBusca, fornecedorId, dataInicio, dataFim], (errSp, saidas) => {
                            if (errSp) return res.status(500).json({ error: errSp.message });

                            const totalChamados = (chamados || []).reduce((acc, c) => acc + (Number(c.custo_servico) || 0), 0);
                            const totalOrcamentosAprovados = (orcamentos || [])
                                .filter(o => o.status === 'Aprovado Financeiro')
                                .reduce((acc, o) => acc + (Number(o.valor_total) || 0), 0);
                            const totalNotasFiscais = (notas || []).reduce((acc, n) => acc + (Number(n.valor_total) || 0), 0);
                            const totalDespesasPrediais = (despesas || []).reduce((acc, d) => acc + (Number(d.valor_total) || 0), 0);

                            return res.json({
                                fornecedor,
                                periodo: { data_inicio: dataInicio, data_fim: dataFim },
                                resumo_financeiro: {
                                    total_gasto_chamados: totalChamados,
                                    total_orcamentos_aprovados: totalOrcamentosAprovados,
                                    total_notas_fiscais: totalNotasFiscais,
                                    total_despesas_prediais: totalDespesasPrediais,
                                    total_saidas_prontuario: (saidas || []).length
                                },
                                chamados: chamados || [],
                                orcamentos: orcamentos || [],
                                notas_fiscais: notas || [],
                                despesas_prediais: despesas || [],
...
app.get('/api/relatorios/exportar/fornecedor/:id', permitirApenas(['admin', 'coordenador', 'tecnico']), async (req, res) => {
    ...
    try {
        ...
        const [rowsForn] = await pool.promise().query(
            "SELECT *, COALESCE(nome_fantasia, razao_social) AS nome_referencia FROM fornecedores WHERE id = ?",
            [fornecedorId]
        );

        if (!rowsForn || rowsForn.length === 0) {
            return res.status(404).json({ error: 'Fornecedor não encontrado.' });
        }
        ...
        const [
            ...
            // Saídas pelo Prontuário / Custódia Externa
            pool.promise().query(`
                ...
                WHERE (eh.descricao_log LIKE CONCAT('%', ?, '%') OR eh.descricao_log LIKE CONCAT('%Fornecedor ID: ', ?, '%'))
                  ...
                  AND DATE(eh.data_movimentacao) BETWEEN ? AND ?
                ORDER BY eh.data_movimentacao DESC
            `, [nomeBusca, fornecedorId, dataInicio, dataFim]),

            // Orçamentos Externos
            pool.promise().query(`
                SELECT o.codigo_orcamento, o.data_emissao, o.tipo_orcamento, o.status, o.valor_total, o.observacoes
                FROM orcamentos_externos o
                WHERE o.fornecedor_id = ? AND DATE(o.data_emissao) BETWEEN ? AND ?
                ORDER BY o.data_emissao DESC
            `, [fornecedorId, dataInicio, dataFim]),

            // Notas Fiscais
            pool.promise().query(`
                SELECT numero_nf, serie, data_emissao, data_recebimento, status, valor_total, descricao
                FROM notas_fiscais
                WHERE fornecedor_id = ? AND DATE(data_emissao) BETWEEN ? AND ?
                ORDER BY data_emissao DESC
            `, [fornecedorId, dataInicio, dataFim]),

            // Despesas Prediais
            pool.promise().query(`
                SELECT dp.codigo_referencia, dp.categoria, dp.descricao, dp.valor_total, dp.forma_pagamento, dp.data_competencia, dp.status_pagamento
                FROM despesas_prediais dp
                WHERE dp.fornecedor_id = ? AND DATE(dp.data_competencia) BETWEEN ? AND ?
                ORDER BY dp.data_competencia DESC
            `, [fornecedorId, dataInicio, dataFim])
        ]);
        ...
        const aplicarCabecalho = (ws, corArgb) => {
            ws.getRow(1).eachCell(cell => {
                cell.font = { bold: true, color: { argb: 'FFFFFFFF' } };
                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: corArgb } };
                cell.alignment = { vertical: 'middle', horizontal: 'center' };
            });
        };

        // =================================================================
        // ⭐ ABA 1: EXTRATO GERAL UNIFICADO (TUDO CRONOLÓGICO)
        // =================================================================
        const wsGeral = workbook.addWorksheet('Extrato Geral');
        wsGeral.columns = [
            { header: 'Tipo de Registro', key: 'origem', width: 24 },
            { header: 'ID / Ref', key: 'referencia', width: 15 },
            { header: 'Data Evento', key: 'data', width: 14 },
            { header: 'Retorno', key: 'data_retorno', width: 14 },
        ...
        (orcamentos || []).forEach(o => {
            listaUnificada.push({
                origem: 'Orçamento Comercial',
                referencia: o.codigo_orcamento,
                data_raw: new Date(o.data_emissao),
                data: new Date(o.data_emissao).toLocaleDateString('pt-BR'),
                data_retorno: '---',
        ...
        (notas || []).forEach(n => {
            listaUnificada.push({
                origem: 'Nota Fiscal',
                referencia: `NF #${n.numero_nf}`,
                data_raw: new Date(n.data_emissao),
                data: new Date(n.data_emissao).toLocaleDateString('pt-BR'),
                data_retorno: n.data_recebimento ? new Date(n.data_recebimento).toLocaleDateString('pt-BR') : '---',
        ...
        (chamadosReais || []).forEach(c => {
            const row = wsChamados.addRow({
                ...
                data_conclusao: c.data_conclusao ? new Date(c.data_conclusao).toLocaleDateString('pt-BR') : 'Pendente',
        ...
        wsSaidas.columns = [
            { header: 'Data/Hora Saída', key: 'data_saida', width: 20 },
            { header: 'Data Retorno', key: 'data_retorno', width: 20 },
            { header: 'Equipamento', key: 'equipamento', width: 30 },
            { header: 'Patrimônio', key: 'patrimonio', width: 14 },
            { header: 'Setor Origem', key: 'setor', width: 20 },
            { header: 'Custódia Atual', key: 'custodia', width: 24 },
            { header: 'Técnico Responsável', key: 'tecnico', width: 22 },
            { header: 'Motivo / Detalhes da Saída', key: 'motivo', width: 50 }
        ];
        ...
        // =================================================================
        const wsOrcamentos = workbook.addWorksheet('Orçamentos & Lotes');
        wsOrcamentos.columns = [
            { header: 'Código Lote', key: 'codigo_orcamento', width: 18 },
            { header: 'Data Emissão', key: 'data_emissao', width: 16 },
            { header: 'Tipo', key: 'tipo_orcamento', width: 16 },
            { header: 'Status Financeiro', key: 'status', width: 22 },
            { header: 'Observações / Proposta', key: 'observacoes', width: 45 },
            { header: 'Valor Total (R$)', key: 'valor_total', width: 18 }
        ];
        aplicarCabecalho(wsOrcamentos, 'FF059669'); // Esmeralda

        (orcamentos || []).forEach(o => {
            const row = wsOrcamentos.addRow({
                codigo_orcamento: o.codigo_orcamento,
                data_emissao: o.data_emissao ? new Date(o.data_emissao).toLocaleDateString('pt-BR') : '',
        ...
        // 🧾 ABA 5: NOTAS FISCAIS
        // =================================================================
        const wsNotas = workbook.addWorksheet('Notas Fiscais');
        wsNotas.columns = [
            { header: 'Número NF', key: 'numero_nf', width: 16 },
            { header: 'Série', key: 'serie', width: 10 },
            { header: 'Data Emissão', key: 'data_emissao', width: 16 },
            { header: 'Data Recebimento', key: 'data_recebimento', width: 18 },
            { header: 'Status', key: 'status', width: 16 },
            { header: 'Descrição / Material', key: 'descricao', width: 40 },
            { header: 'Valor Total (R$)', key: 'valor_total', width: 18 }
        ];
        aplicarCabecalho(wsNotas, 'FF0284C7'); // Azul Céu
        ...
        console.error("❌ Erro ao exportar auditoria completa XLSX:", err);
